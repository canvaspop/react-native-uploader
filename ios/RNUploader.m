        ...
        if( [_file[@"filepath"] hasPrefix:@"assets-library:"]) {
            ...
            [library assetForURL:assetURL resultBlock:^(ALAsset *asset) {
                ...
                if([MIMEType rangeOfString:@"video" options:NSRegularExpressionSearch].location != NSNotFound){
                    ...
                    CGSize size = CGSizeMake(750, 750);
                    UIImage *croppedImage = [self imageByCroppingImage:[UIImage imageWithCGImage:fullScreenImageRef] toSize: size];
                    UIImage *image = [self _square_resize: croppedImage.CGImage];
